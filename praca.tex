...
Przy tworzeniu pracy wykorzystano duże modele językowe (ang. LLM -- \textit{Large Language Models}) do przyspieszenia procesu edycji oraz znajdowania błędów edytorskich i merytorycznych oraz dokonywania drobnych korekt tekstu. Wykorzystanie LLM ograniczono wyłącznie do zadań redakcyjnych, bez ingerencji w merytoryczną treść pracy. W szczególności, tekst pracy nie był generowany przez LLM. Ostateczna wersja tekstu została zweryfikowana i zatwierdzona przez autora jako jego własne opracowanie.
...
Standardowe przypisanie proceduralne w Verilogu/SystemVerilogu, takie jak to pokazane w kodzie \ref{lst:proc-assign}, jest analizowane przez narzędzie do syntezy, które na tej podstawie wnioskuje:
\begin{itemize}
...
\begin{minted}[linenos,breaklines,fontsize=\footnotesize]{systemverilog}
module top;
    reg a=0,b=1;
    reg [1:0] r;
    initial begin
...
\\
\end{tabular}
\codecaption{lst:hierarchy}{Zilustrowanie hierarchii priorytetów przypisań w Verilogu/SystemVerilogu. Instrukcja w linii 6 to zwykłe przypisanie proceduralne, które jest następnie przykrywane przez przypisanie proceduralne kombinacyjne z linii 7. Przypisanie proceduralne w linii 8 jest ignorowane, ponieważ przypisanie proceduralne kombinacyjne jest nadal aktywne. Po wywołaniu \texttt{deassign} w linii 9, przypisanie proceduralne kombinacyjne zostaje wyłączone i kolejne przypisanie proceduralne w linii 10 ma efekt. W linii 11 ponownie wykonywane jest przypisanie proceduralne kombinacyjne. W liniach 12-13 zmieniana jest wartość prawej strony przypisania \texttt{assign}, co powoduje aktualizację wartości \texttt{r}. W linii 14 wykonywane jest przypisanie \texttt{force}, które przykrywa wszystkie poprzednie przypisania. W liniach 15-16 zmieniane są wartości \texttt{a} i \texttt{b}, co powoduje aktualizację wartości \texttt{r}. Przypisania z linii 17-18 są przykryte przez ciągle aktywne przypisanie \texttt{force}. Po wywołaniu \texttt{release} w linii 19, przypisanie \texttt{force} zostaje wyłączone, na skutek czego przypisanie proceduralne kombinacyjne z linii 17 zostaje odkryte i reaguje na zmianę wartości \texttt{b} w linii 20.}
...
\begin{minted}[linenos, fontsize=\footnotesize]{systemverilog}
logic [1:0] a;
...
Pomysłem\footnote{https://github.com/ArturBieniek4/verilator/tree/81646-force-rework-v4} na usprawnienie poprzedniej implementacji, umożliwiając verilację i przetestowanie wydajności symulacji było zastąpienie złożonych wyrażeń w drzewie AST pętlami \texttt{while} (identycznymi jak te dostępne w Verilogu), które w czasie działania symulacji będą wykonywały odpowiednie przypisania i zwracały wartość wymuszoną. Ogranicza to rozrost drzewa AST na etapie verilacji kosztem zwiększenia liczby instrukcji wykonywanych w czasie symulacji.
...
\end{minted}
\codecaption{lst:dpi-force-transformation}{Przekształcenie kodu \ref{lst:dpi-force} wykonane przez Verilator, pozyskane przy użyciu flag \texttt{----dump-tree ----debug-emitv}, uproszczone i sformatyzowane ręcznie dla czytelności.}
